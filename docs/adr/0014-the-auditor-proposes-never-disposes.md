...
- **Status:** Accepted
- **Date:** 2026-08-07
- **Source:** Grilling 3, Q1/Q4, in `docs/PROMPT_TRAIL.md` Session 14

## Context

The Inventory Auditor must flag the Dell XPS, the MacBook Air, the 2027 date, id 10
and `"Appel"`. `needs_review` is a rentability guard (ADR-0003), so an auditor that
sets it is an LLM changing what may be rented.

## Decision

**The auditor's vocabulary is a closed enum of finding kinds**:
`status_contradiction`, `unidentifiable` and `probable_misspelling`, exactly the three
...
- Nothing persisted: no new table for the demo reset to erase, no staleness
  question. Findings gain an actor only when one becomes a flag through ADR-0017, a
...
- **Amended 2026-08-07 (post-gate):** an in-memory cache keyed on the catalogue
  fingerprint serves repeated runs against an unchanged inventory, because the free tier
