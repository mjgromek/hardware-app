...
already naming seed id 11's history as the loop it closes — the framing is written, the
feature is not.
...
(`styles.css:149`). Blue does appear nowhere else in the *palette* — it appears
...
**2.4.11 Focus Appearance** question, not a taste question — the new indicator must
reach **3:1** against both the unfocused state and the surrounding surface. Measure it
into `docs/ACCESSIBILITY.md` in the same pass, or the palette complaint is traded for an
accessibility regression that no test would catch.

**3. Status column centred — NOT STARTED.**
`HardwareTable.vue:208` is `<td><StatusChip :status="item.status" /></td>` — no class,
...
about 2.6× the AA requirement of 4.5:1. There is no headroom — darkening it further is
...
`npm run build` before `uvicorn`, but a reviewer whose first instinct is `pytest` — the
instinct this project has trained — hits a failure the README never warns about. That is
the most likely first failing step. Clone, follow it literally, report what breaks.
...
outstanding — a second "make it larger / add more padding" pass without a target number
...
5. **Items 4 and 6**, visual confirmation only — no code unless a number is named.
...
still happens — an unshipped v4 is the one outcome that makes the rest of this pointless.
